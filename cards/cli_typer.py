...
@app.command("list")
def list_cards(noowner: bool = typer.Option(None, "-n", "--noowner", "--no-owner", help="list cards without owners"),
               owner:str = typer.Option(None, "-o", "--owner", help="filter on the cards owner"),
               priority:int = typer.Option(None, "-p", "--priority", help="fliter on this priority and above"),
               done:bool = typer.Option(None, "-d", "--done", help="filter on cards with given done state"),
               format:str = typer.Option(DEFAULT_TABLEFORMAT, "-f", "--format", help='table fomratting option, eg. "grid", "simple", "html"'),
               ):
    """
    List cards in db.
    """
...
def update(card_id: int,
           ...
           priority:int = typer.Option(None, "-p", "--priority", help="change the card priority"),
           summary:List[str] = typer.Option(None, "-s", "--summary", help="change the card summary"),
           done:bool = typer.Option(None, "-d", "--done", help="change the card done state (True or False)"),
           ):
...
@app.command()
def count(noowner: bool = typer.Option(None, "-n", "--noowner", "--no-owner", help="count cards without owners"),
          owner:str = typer.Option(None, "-o", "--owner", help="count cards with given owner"),
          priority:int = typer.Option(None, "-p", "--priority", help="count cards with given priority"),
          done:bool = typer.Option(None, "-d", "--done", help="count cards with given done state (True or False)"),
          ):
